...
- **Lucide icons — secondary admin surfaces (not yet started):** Follow-up pass for `AdminHandicap` (⚙️, 🔒), `AdminLeague` (📅), `AdminPlayers` (👤, 🔒/🔓, 🎯, 📊, 🤝), `AdminSchedule` (🎯, 🔒, ⛳), `AdminSubs` (⚠️). Toast strings (`✅ ...`) are deferred until we build a shared `Toast` component with a proper icon slot — doing them inline now means two passes through the same strings.
